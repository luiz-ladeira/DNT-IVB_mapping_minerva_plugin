...
function fillPanelHeight(rootEl) {
  try {
    ...
    const drawer =
      $(mount).closest(".tab-content").get(0) || mount.parentElement || mount;
    ...
    const top = renderUI._anchorEl.getBoundingClientRect().top;
    const dr = drawer.getBoundingClientRect();
    const left = dr.left;
    const width = dr.width;
    const height = window.innerHeight - top - PANEL_BOTTOM_GAP;
